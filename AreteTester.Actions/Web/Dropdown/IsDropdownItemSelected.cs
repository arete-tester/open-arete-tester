...
{
    [Serializable]
    public class IsDropdownItemSelected : ActionBase
    {
        [XmlAttribute]
        [Category("Properties")]
        ...
        public IsDropdownItemSelected()
        {
            this.ActionType = "IsDropdownItemSelected";
            this.Variable = "@var";
        }
        ...
        public override void Process()
        {
            ...
            bool selected = false;
            foreach (var option in element.AllSelectedOptions)
            {
                if (String.IsNullOrEmpty(option.Text) == false && option.Text == this.Text)
                {
                    selected = true;
                    break;
                }
            ...
            Variables.Instance.SetValue(Variable, selected);
