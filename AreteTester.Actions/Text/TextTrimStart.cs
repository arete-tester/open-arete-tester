...
{
    [Serializable]
    public class TextTrimStart : ActionBase
    {
        [XmlAttribute]
        [Category("Properties")]
        ...
        public override void Process()
        {
            ...
            {
                if (String.IsNullOrEmpty(TrimChars) == false)
                {
                    Variables.Instance.SetValue(this.Text, text.TrimEnd(this.TrimChars.ToCharArray()));
                }
                else
                {
                    Variables.Instance.SetValue(this.Text, text.TrimEnd());
                }
            }
